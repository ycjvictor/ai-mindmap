...
class parser extends utils {
	...
	rootFormData(data, imgFiles) {
		...
		const markerList = markerRefs['marker-refs'] || markerRefs['marker-ref'] || markers;
		...
		Array.isArray(markerList) &&
			markerList.forEach((v) => {
				...
				if (markerId.indexOf('priority') >= 0) {
					const num = Number(markerId.replace(/[^0-9]/gi, ''));
		...
		const params = {
			...
			data: {
				...
				priority,
				progress,
				note: notes.plain.content || notes.plain._text,
				image: imageBase,
				imageSize,
			},
		};
	...
	async xmindToJSON(data) {
		try {
			...
			if (kityminderFile) {
				...
				//获取解压包的图片列表
				const imglist = Object.keys(files).filter((v) => {
					// this.imgRegex.test(path.basename(v));
					if (path.dirname(v) == 'attachments' || path.dirname(v) == 'resources') {
						return true;
					}
				});
